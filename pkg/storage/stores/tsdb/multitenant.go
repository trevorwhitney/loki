...
func (m *MultiTenantIndex) Versions(ctx context.Context, userID string, from, through model.Time, acc IndexVersionAccumulator) error {
	return m.idx.Versions(ctx, userID, from, through, acc)
}
